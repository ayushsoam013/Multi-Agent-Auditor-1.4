...
def test_prefix_normalization_with_provider_path():
    ...
    assert result == "litellm_proxy/google/gemini-2.5-flash", (
        f"Expected litellm_proxy/google/gemini-2.5-flash, got {result}"
    )

    # Test with openai model
    result = service._ensure_litellm_proxy_prefix("openai/gpt-4o-mini")
    assert result == "litellm_proxy/openai/gpt-4o-mini", (
        f"Expected litellm_proxy/openai/gpt-4o-mini, got {result}"
    )
...
def test_prefix_normalization_already_has_prefix():
    ...
    result = service._ensure_litellm_proxy_prefix(
        "litellm_proxy/google/gemini-2.5-flash"
    )
    assert result == "litellm_proxy/google/gemini-2.5-flash", (
        f"Expected litellm_proxy/google/gemini-2.5-flash, got {result}"
    )
...
def test_default_model_names():
    ...
    assert service.model_name == "google/gemini-2.5-flash-lite", (
        f"Expected google/gemini-2.5-flash-lite, got {service.model_name}"
    )

    # When used, it should get the prefix
    prefixed = service._ensure_litellm_proxy_prefix(service.model_name)
    assert prefixed == "litellm_proxy/google/gemini-2.5-flash-lite", (
        f"Expected litellm_proxy/google/gemini-2.5-flash-lite, got {prefixed}"
    )
